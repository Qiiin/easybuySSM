    ...
    <!--Begin 所在收货地区 Begin-->
        ...
        <span class="fr">
          <c:if test="${sessionScope.loginUser==null}">
            <span class="fl">你好，请<a href="${ctx}/pre/user/login.jsp"  style="color:#ff4e00;">登录</a>&nbsp;<a href="${ctx}/pre/user/regist.jsp" style="color:#ff4e00;">免费注册</a>&nbsp;&nbsp;</span>
          </c:if>
          <c:if test="${sessionScope.loginUser!=null}">

            <span class="fl">
            	<a href="${ctx}/backend/member.jsp">${sessionScope.loginUser.loginName}</a>&nbsp;
            	<c:if test="${sessionScope.loginUser.type!=1}">
            		|&nbsp;<a href="${ctx}/pre/OrderServlet?action=toOrderManage&userId=${sessionScope.loginUser.id}">我的订单&nbsp;</a>
            	</c:if>
            </span>

          </c:if>
           <c:if test="${sessionScope.loginUser!=null && sessionScope.loginUser.type==1}">
            <span class="fl">|&nbsp;<a href="${ctx}/backend/productServlet?action=index&userId=${sessionScope.loginUser.id}">后台管理</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null}">
             <span class="fl">|&nbsp;<a href="${ctx}/pre/userServlet?action=exit" >注销</a></span>
          </c:if>
        </span>
  </div>
</div>
