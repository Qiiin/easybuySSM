...
<%@ page import="com.easybuy.entity.Category"%>
<%@ page import="java.util.List"%>
<%@ page import="com.easybuy.util.EmptyUtils"%>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>

<%
	List<Category> vo1List=(ArrayList)request.getAttribute("vo1List");
	if(EmptyUtils.isEmpty(vo1List)){
...
<div class="menu_bg">
    <div class="menu">
                            ...
                            <div class="zj">
                                <div class="zj_l">
                                    <c:forEach items="${temp.categoryVoList}" var="vo">
                                        <div class="zj_l_c">
                                            <h2>
                                                <a href="${ctx}/pre/productServlet?action=index&categoryLevelId=${vo.category.id}">${vo.category.name}</a>
                                            </h2>
                                            <c:forEach items="${vo.categoryVoList}" var="vo2">
                                                <a href="${ctx}/pre/productServlet?action=index&categoryLevelId=${vo2.category.id}">${vo2.category.name}</a> |
                                            </c:forEach>
                                        </div>
                                    </c:forEach>
                                </div>
        ...
        <ul class="menu_r">
            ...
            <c:forEach items="${vo1List}" var="temp">
                <li><a href="${ctx}/pre/productServlet?action=index&categoryLevelId=${temp.category.id}">${temp.category.name}</a></li>
            </c:forEach>
        </ul>
...
<div class="i_bg bg_color">
    <div class="i_ban_bg">
        ...
        <div class="inews">
            <div class="news_t">
                <span class="fr"><a href="${ctx}/pre/newsServlet?action=queryNewsList">更多 ></a></span>新闻资讯
            </div>
            <ul>
                <c:forEach items="${news}" var="temp">
                    <li><span>${temp.type}</span>
                        <a href="${ctx}/pre/newsServlet?action=newsDeatil&id=${temp.id}">${temp.title}</a>
                    </li>
                </c:forEach>
            </ul>
            <div class="charge_t">
