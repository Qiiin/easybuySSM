...
<div class="top">
    ...
    <div class="search">
        <form action="${ctx}/pre/productServlet?action=index" method="post">
            <input type="text" value="${keyWord}" name="proName" class="s_ipt">
	        ...
	        <div class="car_bg">
	            ...
	            <ul class="cars">
	                <c:forEach items="${sessionScope.cart.items}" var="temp">
	                    <li>
	                        <div class="img"><a href="${ctx}/pre/productServlet?action=getProductDetail&id=${temp.product.id}"><img src="${ctx}/files/${temp.product.fileName}" width="58" height="58" /></a></div>
	                        <div class="name"><a href="${ctx}/pre/productServlet?action=getProductDetail&id=${temp.product.id}">${temp.product.name}</a></div>
	            ...
	            <c:if test="${sessionScope.loginUser==null}">
	                <div class="price_a"><a href="${ctx}/pre/user/login.jsp">去登录</a></div>
	            </c:if>
	            <c:if test="${sessionScope.loginUser!=null}">
	                <div class="price_a"><a href="${ctx}/pre/CartServlet?action=toSettlement">去结算</a></div>
	            </c:if>
